  ...
  return (
    <div>
      <h2 id="autor-heading" data-cy="AutorHeading">
        <Translate contentKey="miPrimeraApp.autor.home.title">Autors</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="miPrimeraApp.autor.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/autor/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="miPrimeraApp.autor.home.createLabel">Create new Autor</Translate>
          </Link>
        </div>
      </h2>
        ...
        {autorList && autorList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="miPrimeraApp.autor.id">ID</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                </th>
                ...
                <th>
                  <Translate contentKey="miPrimeraApp.autor.user">User</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {autorList.map((autor, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{autor.nombre}</td>
                  <td>{autor.email}</td>
                  <td>{autor.user ? autor.user.id : ''}</td>
                  <td className="text-end">
                    <div className="btn-group flex-btn-group-container">
                      ...
                      <Button
                        onClick={() => (window.location.href = `/autor/${autor.id}/delete`)}
                        color="danger"
                        size="sm"
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="miPrimeraApp.autor.home.notFound">No Autors found</Translate>
            </div>
          )
        )}
      </div>
